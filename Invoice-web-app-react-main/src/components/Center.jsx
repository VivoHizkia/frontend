...
function Center() {
    ...
    useEffect(() => {
        if (selectedMenu === 'invoices') {
            dispatch(invoiceSlice.actions.filterInvoice({ status: '' })); // Tampilkan semua invoice
        } else if (selectedMenu === 'payments') {
            dispatch(invoiceSlice.actions.filterInvoice({ status: 'paid' })); // Tampilkan hanya invoice yang sudah dibayar
        }
                ...
                <table className="min-w-full bg-white dark:bg-[#1E2139] rounded-lg">
                    <thead className="bg-gray-100 dark:bg-[#252945] text-sm text-gray-600 dark:text-gray-300">
                        <tr>
                            ...
                            <th className="text-center py-4 px-6">Void</th>
                        </tr>
                    </thead>
                    <tbody>
                        <AnimatePresence>
    ...
    return (
        <div className="flex">
                ...
                >
                    <div className="min-w-full max-h-[64px] flex items-center justify-between">
                        ...
                        <div className="flex max-h-full items-center">
                            ...
                            {selectedMenu === 'payments' && (
                                <button
                                    onClick={() => setOpenCreatePayment(true)}  // Tambahkan event handler untuk New Payment
                                    className="hover:opacity-80 ml-4 md:ml-10 flex items-center py-2 px-2 md:space-x-3 space-x-2 bg-[#7c5dfa] rounded-full"
                                >
                                    <img src={plus} alt="" />
            ...
            <AnimatePresence>
                ...
                {openCreatePayment && (  // Tambahkan CreatePayment di sini
