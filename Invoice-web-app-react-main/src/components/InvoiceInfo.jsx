    ...
    <div>
      {invoice ? (
        ...
        >
          ...
          {/* STATUS BAR */}
          <div className='mt-2 rounded-lg w-full flex items-center justify-between px-6 py-6 bg-white dark:bg-[#1e2139]'>
            ...
            <div className='hidden md:flex space-x-3'>
              <button
                ...
                className='text-[#7e88c3] bg-slate-100 dark:bg-[#252945] hover:opacity-80 p-3 px-7 rounded-full'
              >Edit</button>
              <button
              ...
              {invoice.status === 'pending' && (
                <button
                  ...
                  className='text-white bg-[#7c5dfa] hover:opacity-80 p-3 px-7 rounded-full'
                >Make as Paid</button>
              )}
            </div>
          </div>
          ...
          <div
            ...
            className="mt-6 bg-white dark:bg-[#f8f8fb] p-6 shadow-md text-black font-sans"
          >
            {/* Header & Logo */}
            ...
            <div className="border-t border-b">
              <div className="grid grid-cols-5 font-semibold bg-gray-100 p-2">
              ...
              {invoice.items.map((item, idx) => (
                <div key={idx} className="grid grid-cols-5 p-2 border-t text-sm">
                  <p className="col-span-1">{item.quantity}</p>
                  <div className="col-span-2">
                    <p className="font-medium">{item.name}</p>
                    <p className="text-gray-500 text-xs">{item.description || '-'}</p>
                  </div>
                  <p className="col-span-1 text-right">{formatCurrency(item.price, invoice.currency || 'USD')}</p>
                  <p className="col-span-1 text-right">{formatCurrency(item.total, invoice.currency || 'USD')}</p>
                </div>
              ))}
            </div>
            ...
            <div className="text-right mt-6 space-y-2 text-sm">
              <p><span className="font-bold">Subtotal:</span> {formatCurrency(invoice.total, invoice.currency || 'USD')}</p>
              <p><span className="font-bold">Tax :</span> -</p>
              <p className="text-lg font-bold"><span>Total:</span> {formatCurrency(invoice.total, invoice.currency || 'USD')}</p>
            </div>
