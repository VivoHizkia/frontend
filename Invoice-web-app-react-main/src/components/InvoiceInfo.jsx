            ...
            <div className="border-t border-b">
              ...
              {invoice.items.map((item, idx) => (
                <div key={idx} className="grid grid-cols-6 p-2 border-t text-sm">
                  <p className="col-span-1">{formatNumber(item.quantity || 0)}</p>
                  <div className="col-span-2">
                    <p className="font-medium">{item.name}</p>
                    <p className="text-gray-500 text-xs">{item.description || '-'}</p>
                  </div>
                  <p className="col-span-1 text-right">{formatNumber(item.usage || 0)}</p>
                  <p className="col-span-1 text-right">{formatCurrency(item.price || 0, invoice.currency || 'USD')}</p>
                  <p className="col-span-1 text-right">
                    {formatCurrency((item.usage || 0) * (item.price || 0), invoice.currency || 'USD')}
                  </p>
                </div>
              ))}
            </div>
            ...
            <div className="text-right mt-6 space-y-2 text-sm">
              ...
              <p className="text-lg font-bold">
                <span>Total:</span> {formatCurrency(
                  invoice.items.reduce((sum, item) => sum + (item.usage || 0) * (item.price || 0), 0) +
                  (invoice.tax ? invoice.items.reduce((sum, item) => sum + (item.usage || 0) * (item.price || 0), 0) * (invoice.tax / 100) : 0),
                  invoice.currency || 'USD'
                )}
              </p>
            </div>
